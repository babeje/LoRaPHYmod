...
% --- График 3а: SNR по хопам маршрута ---
if nHops > 1
    tickLbls = arrayfun(@(h) sprintf('%d→%d', route(h), route(h+1)), 1:nHops, 'UniformOutput', false);

    figure('Name','SNR по хопам','Color','w','Position',[90 90 720 420]);
    bar(1:nHops, routeSNR, 0.55, 'FaceColor', colors(1,:), 'FaceAlpha', 0.8);
    hold on;
    % Линия порога чувствительности
    yline(snrThreshold, '--r', 'LineWidth', 1.5, 'Label', sprintf('Порог SF%d', sf));
    set(gca, 'XTick', 1:nHops, 'XTickLabel', tickLbls, 'XTickLabelRotation', 30, 'FontSize', 12);
    xlabel('Хоп маршрута', 'FontSize', 13);
    ylabel('SNR, дБ', 'FontSize', 13);
    title(sprintf('SNR по хопам | маршрут %d→%d (%d хопов)', src, dst, nHops), 'FontSize', 13);
    grid on; hold off;

    % --- График 3б: PER vs SNR по хопам маршрута ---
    figure('Name','PER vs SNR по хопам','Color','w','Position',[110 110 680 480]);
    scatter(routeSNR, hopPER, 90, colors(2,:), 'filled', 'MarkerEdgeColor', 'k', 'LineWidth', 0.8);
    hold on;
    % Подписи хопов рядом с точками
    for h = 1:nHops
        text(routeSNR(h) + 0.3, hopPER(h) + 0.02, ...
            sprintf('Хоп %d', h), 'FontSize', 10, 'Color', [0.2 0.2 0.2]);
    end
    xline(snrThreshold, '--r', 'LineWidth', 1.5, 'Label', sprintf('Порог SF%d', sf));
    xlabel('SNR хопа, дБ', 'FontSize', 13);
    ylabel('PER хопа', 'FontSize', 13);
    title(sprintf('PER vs SNR по хопам | PER_{e2e} = %.4f', PER_e2e), 'FontSize', 13);
    grid on; ylim([0, max(hopPER)*1.3 + 0.05]); hold off;
end
...
text(X(src)+20, Y(src)+20, Z(src)+1, sprintf('src=%d', src), 'FontSize', 10, 'FontWeight', 'bold');
text(X(dst)+20, Y(dst)+20, Z(dst)+1, sprintf('dst=%d', dst), 'FontSize', 10, 'FontWeight', 'bold');

colormap(parula); cb = colorbar;
cb.Label.String = 'Высота узла, м'; cb.Label.FontSize = 11;

xlabel('X, м', 'FontSize', 12); ylabel('Y, м', 'FontSize', 12); zlabel('Высота, м', 'FontSize', 12);
title(sprintf('3D-сеть | маршрут %d→%d | %d хопов | цвет хопа = PER', src, dst, nHops), 'FontSize', 13);
